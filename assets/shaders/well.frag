...
void main() {
    ...
    //float btime = u_time / 3500.0;


    //vec4 maxed = max(max(max(texture2D(u_texture, T + vec2(0, stroke) * v), texture2D(u_texture, T + vec2(0, -stroke) * v)), texture2D(u_texture, T + vec2(stroke, 0) * v)), texture2D(u_texture, T + vec2(-stroke, 0) * v));
