...
float sinf(float d) {
    return (sin((u_time / 40.0)*0.2 + d) + sin((u_time / 20.0) * 0.2 + d)*0.5 + sin((u_time / 90.0)*0.2 + d)*2.0 + sin((u_time / 180.0)*0.2 + d)*3.0)*10.0;
}
...
void main(){
    ...
    float noise_1 = (texture(u_noise, (c) / noiseScl + vec2(btime) * vec2(-0.9, 0.8)).r + texture(u_noise, (c) / noiseScl + vec2(btime * 1.1) * vec2(-0.8, -1.0)).r) / 2.0;
    float noise_2 = (texture(u_noise, (c) / (noiseScl*5.5) + vec2(btime) * vec2(-0.9, 0.8)).r + texture(u_noise, (c) / (noiseScl*5.5) + vec2(btime * 1.1) * vec2(-0.8, -1.0)).r) / 2.0;
    ...
    float noiseO = (texture(u_noise, o / (noiseScl*0.02) + vec2(btime) * vec2(-0.9, 0.8)).r + texture(u_noise, o / noiseScl/2.0 + vec2(btime * 1.1) * vec2(-0.8, -1.0)).r) / 2.0;
    ...
    color.rgb *= alpha;
    gl_FragColor = color;  // 如果 Mindustry 基类没处理，这里要改成自定义 out 变量
}