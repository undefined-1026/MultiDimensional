...
public class BoostWeapon extends Weapon {
    ...
    public float keepWarmupTime = 60f;
    ...
    public void update(Unit unit, WeaponMount m){
        ...
        mount.boostProcess = Mathf.approachDelta(mount.boostProcess,m.warmup>0.05f?1f:0,(m.shoot?warmupSpeed:dissipateSpeed) * Time.delta);
        ...
        float warmupTarget = mount.keepWarmup > 0||(can && mount.shoot) || (continuous && mount.bullet != null) || mount.charging ? 1f : 0f;
        if((can && mount.shoot)){
            mount.keepWarmup = keepWarmupTime;
        }else{
            mount.keepWarmup = Mathf.approachDelta(mount.keepWarmup, 0, Time.delta);
        }
        if(linearWarmup){
