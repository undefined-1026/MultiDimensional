...
import arc.util.Tmp;
import mDimension.entity.ability.chargeAbility;
...
public class chargeWeapon extends Weapon {
    public int chargeAbilityIndex = 0;
    public boolean mustConsume = true;
    public float consumption = 100f;
    public float damageBoost = 0;
    public float reloadBoost = 0;
    ...
    public void update(Unit unit, WeaponMount mount){
        boolean cann = true;
        float rBoost = 0;
        if(unit.abilities()[chargeAbilityIndex] instanceof chargeAbility a){
            cann = a.data>consumption || !mustConsume;
            rBoost = a.efficiency() * reloadBoost;
        }
        boolean can = cann && unit.canShoot();
        float lastReload = mount.reload;
        mount.reload = Math.max(mount.reload - Time.delta * unit.reloadMultiplier * (1+rBoost), 0);
    ...
    protected void handleBullet(Unit unit, WeaponMount mount, Bullet bullet) {
        ...
        if(damageBoost>0 && unit.abilities()[chargeAbilityIndex] instanceof chargeAbility a){
            bullet.damage*=(1+damageBoost*a.efficiency());
        }
    }
}
